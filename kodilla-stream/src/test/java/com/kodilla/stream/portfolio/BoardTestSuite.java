...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        List<Integer> createTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(n -> n.getTasks().stream())
                .map(Task::getCreated)
                .map(n -> n.getDayOfYear())
                .toList();

        double averangeTimeOnTask = IntStream.range(0, createTask.size())
                .map(n-> LocalDate.now().getDayOfYear() - createTask.get(n))
                .average().getAsDouble();
        //Then

        assertEquals(10.0, averangeTimeOnTask);
    }
